...
If you notice any bugs, or have any feature requests please make a [github issue](https://github.com/Varrox/Quick-Add-Menu/issues) describing it so I can implement the feature or a fix.
...
func add_custom_items(): ## Add custom items
    ...
    var item = Item.new("Rigid Body", get_icon("RigidBody3D"), create_rigidbody)

    node_3d_list.insert(15, item) # inserting at the 15th item to be before the nodes group.
```
...
func add_custom_items(): ## Add custom items
    ...
    node_3d_list.insert(15, item) # inserting at the 15th item to be before the nodes group, you don't need to do this, you can just do either append or insert.
```
...
```gdscript
func add_custom_items(): ## Add custom items
    var header = Item.new_header("This is a cool header!")

    node_3d_list.append(header)
```
...
func add_custom_items(): ## Add custom items
    ...
    var create_rigidbody:Callable = func(): return [RigidBody3D.new()] as Array[Node]
    var item = Item.new("Rigid Body", get_icon("RigidBody3D"), create_rigidbody, my_id) # my_id is parent_item_id argument.

    node_3d_list.append(item)
```
...
func add_custom_items(): ## Add custom items
    ...
    var header = Item.new_header("This is a cool header!", null, my_id) # The second argument is null because it is a reference to a texture, which we do not have.
